        ...
        for (int row = 0; row < 20; row++) {
            ...
            if (row<10) {
                for (int element = 0; element <= row; element++) {
                    makeHexa(startingX + (-(row * 15) + (element * 30)), staringY + (row * 10), graphics);
                }
            } 
        }
        // middle part
        for (int row = 0; row < 17; row++) {
            int startingMiddleY = 140;
            if (row%2==0){
                int startingMiddleX = 20;
                for (int element = 0; element < 10; element++) {
                makeHexa(startingMiddleX+(element*30), startingMiddleY+(row*10),graphics);
                }
            }else {
